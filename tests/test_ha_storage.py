...
from aquable.storage.ha_config import HAConfig, HAConfigStorage, HAEntity
...
class TestHAConfigStorage:
    ...
    def test_save_and_load_config(self, temp_storage):
        ...
        entity = HAEntity(entity_id="switch.test", label="Test Switch", type="switch")
    ...
    def test_add_entity(self, temp_storage):
        ...
        success = temp_storage.add_entity(
            entity_id="switch.pump", label="Main Pump", entity_type="switch"
        )
        assert success is True
...
class TestHAEntity:
    ...
    def test_valid_switch_entity(self):
        ...
        entity = HAEntity(entity_id="switch.test", label="Test", type="switch")
        assert entity.entity_id == "switch.test"
        assert entity.type == "switch"

    def test_valid_script_entity(self):
        """Should create valid script entity"""
        entity = HAEntity(entity_id="script.routine", label="Routine", type="script")
